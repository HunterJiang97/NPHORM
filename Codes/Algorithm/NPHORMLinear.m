classdef NPHORMLinear < Algorithm
    ...
    methods
        ...
        function [model_information] = runAlgorithm(obj, train, test, parameters)
                ...
                c1 = clock;
                [model_information.projectedTrain, model_information.predictedTrain] = obj.test(train,train,model);
                [model_information.projectedTest, model_information.predictedTest] = obj.test(train,test,model);
                c2 = clock;
...
%             kertype = obj.kernelType;
            [model.Beta,model.b,model.Kern] = NPHORMlin(train.patterns, train.targets, param.C, param.Cs);
        ...
        function [decv, pred]= test(obj, train, test,model)
          nT=test.nOfPatterns;  
          decv= zeros(nT,train.nOfClasses);         
%           if nT >500
%               for j=1:nT/500
%                   xnewk=test.patterns(500*(j-1)+1:500*j,:);
%                   decv(500*(j-1)+1:500*j,:) = model.Beta' * Kernel(obj.kernelType, xnewk',train.patterns',param.k) + model.b;
%               end
%               xnewk=test.patterns(500*j+1:nT,:);
%               decv(500*j+1:nT,:)= model.Beta' * Kernel(obj.kernelType, xnewk',train.patterns',param.k) + model.b;
%           else
              decv =  test.patterns * model.Beta + model.b;
%           end
